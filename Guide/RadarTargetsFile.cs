...
// the global "*" area block (League Mechanic, Boss Room, Rune, ...) draws in every zone, merged in after the
// area-specific rows. a bare "*" Name with no Rooms is unusable (whole-map) and skipped. Alternatives are
// flattened in, sharing the parent display name.
public static class RadarTargetsFile
{
    ...
    // Radar's ExpectedCount for a Tile (clusters the pattern splits into, e.g. 2 staircases), 1 otherwise. the
    // resolver passes it to ClusterTarget so a multi-instance tile gets per-cluster centroids, not one void midpoint.
