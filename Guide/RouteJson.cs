// one shared serializer for the plugin and tests, so tool-written and runtime-read route.json never drift.
// stays ExileCore2-free so the test project (which compiles Guide/*.cs) can round-trip it.
...
public static class RouteJson
{
    ...
    private static Objective ObjFromJson(JToken t)
    {
        ...
        // v1 back-compat: old objectives stored pathType + tiles. reconstruct guidance children.
    ...
    // v1 objectives used pathType + tiles instead of Paths/Indicators children. rebuild the children with the
    // same tile routing: Interact/Proximity/Talk tiles -> Room, else Tile; every entity matcher -> Entity path + indicator.
