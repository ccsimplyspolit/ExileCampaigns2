...
// step = sequence of fragments (literal text + typed tokens). pure C#. PlainText() is plain-text-first;
// icon/colour can layer on later by switching on Kind.
...
{
    // 8-point compass from degrees (0 = up/N, clockwise) in 45 degree steps.
...
public sealed class ParsedStep
{
    ...
    // consolidated guidance metadata built by LegacyView.ForStep; null when the step has no guidance.
