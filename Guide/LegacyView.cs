...
// builds the ParsedStep (fragments + StepMeta) that StepTargetResolver/Pathing/Indicators consume, from a
// RouteStep. guidance only, advance is native via AdvanceEngine, so a missing mapping (e.g. Loot has no
// path) just means no guidance target, never wrong advance.
public static class LegacyView
{
    public static ParsedStep ForStep(RouteStep m)
    {
        ...
        {
            // guidance is decoupled from Type: any objective can carry any Path child, so we route
            // tile/room/entity-path for every type. the switch handles only the type-specific bits
            // (kill target, advance flag, complete-area, interact-kind label).
            ...
            // live AreaTransition match + the PathTilePatterns channel, never as a borrowable PathTarget.
        ...
        // minimal entity-objective so its ground path still renders. rarely fires.
        ...
        if (flag != null || completeArea != null || pathTile != null
            ...
            // an EnterArea tile is a plain Tile Path child resolved via PathTilePatterns + the live
            // AreaTransition match, not synthesized onto StepMeta here.
            ps.Meta = new StepMeta(
                CompletionFlag: flag, Objective: objMeta,
                PathTarget: pathTile, InteractKind: interactKind,
