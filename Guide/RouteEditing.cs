...
// pure edit helpers over the immutable route records. all list math and record rebuilding lives here so
// it's unit-tested without game state.
public static class RouteEditing
{
    ...
    // sole bare Manual/QuestFlag gets retyped in place (guidance children stay), else append so existing
    // kill/talk/path objectives keep their guidance.
    ...
    // same in-place-vs-append rule as AddQuestFlagObjective.
