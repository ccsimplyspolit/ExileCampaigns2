...
// reads a step's guidance. the on-screen arrow comes from objectives' Indicators[], the ground path from
// Paths[], independently, so an indicator can exist with no path and a path with no arrow.
public static partial class GuidanceView
{
    ...
    // ground path to each authored entity target, so multiple distinct entities each get a route.
    ...
    // variants) and the path lands on whichever is present.
    ...
    // line to each matching AreaGraph room center. gathers across ALL objectives, so rooms on a second objective aren't dropped.
    ...
    // a line per target (All) or defers to the single-target nearest flow (Nearest, the default). a step mixing
    // All and Nearest objectives draws all.
    ...
    // lookahead window: when currentStepId resolves to a step in `steps` and lookahead >= 0, only the current
    // step and the next `lookahead` steps (by route order) draw, so far-future steps in the same area don't
    // clutter the map. lookahead < 0 (or an unknown current step) keeps every same-area step.
    public static IReadOnlyList<MinimapIconSpec> MinimapIconsForArea(IReadOnlyList<RouteStep> steps, string areaId,
        string? currentStepId = null, int lookahead = -1)
    {
        var res = new List<MinimapIconSpec>();
        if (steps == null) return res;

        int currentIdx = -1;
        if (lookahead >= 0 && currentStepId != null)
            for (int i = 0; i < steps.Count; i++)
                if (string.Equals(steps[i].Id, currentStepId, StringComparison.Ordinal)) { currentIdx = i; break; }
        bool gate = currentIdx >= 0;

        for (int idx = 0; idx < steps.Count; idx++)
        {
            var step = steps[idx];
            if (gate && (idx < currentIdx || idx > currentIdx + lookahead)) continue;
