...
// state, plus move/delete the current step, bind EnterArea/quest-flag advances, and set paths from Radar targets
public partial class ExileCampaigns
{
    ...
    private void DrawTriageOverlay()
    {
        ...
        if (vis)
        {
            ...
            // one-click advance bind: set the current step to EnterArea for the area you're standing in
    ...
    // targets the editor's selected objective index (clamped), so pick the right objective in the editor first.
    ...
    // current step (CompleteWhen Any, existing guidance preserved). the ring stays filled whenever this panel
    // is open, so this works without the Log-quest-flags toggle.
    ...
    // append a QuestFlag advance objective for `flag` to the current step, persist, rebuild, and jump the
    // editor onto the new objective so you can verify it
    ...
    // set the current step to advance on entering the live area via an EnterArea objective, persist, rebuild,
    // and jump the editor onto it so you can verify it
    ...
    // add a step before the anchor with "Get Waypoint" text and an ActivateWaypoint objective.
    // inherits the anchor's act/area so it slots in naturally
    ...
    // compact snapshot stuffed into a new step's Note so you can flesh it out later
