...
public class InteractIndicatorSettings
{
    ...
    [Menu("Arrow color")]
    public ColorNode IconColor { get; set; } = new ColorNode(Color.FromArgb(255, 255, 200, 60));
...
public class MinimapIconSettings
{
    ...
    [Menu("Lookahead steps", "Only draw icons for the current step and the next N steps in this area (0 = current step only)")]
...
public class OverlayStyle
{
    ...
    [Menu("Header color")] public ColorNode HeaderColor { get; set; } = new ColorNode(Color.FromArgb(255, 235, 200, 110));
    [Menu("Optional color")] public ColorNode OptionalColor { get; set; } = new ColorNode(Color.FromArgb(255, 150, 150, 150));
    [Menu("Background color", "Alpha controls opacity; alpha 0 = no panel background")]
    public ColorNode BackgroundColor { get; set; } = new ColorNode(Color.FromArgb(140, 8, 8, 12));
    [Menu("Border color")] public ColorNode BorderColor { get; set; } = new ColorNode(Color.FromArgb(180, 90, 90, 110));
...
public class StepsOverlayStyle
{
    ...
    public ColorNode HeaderColor { get; set; } = new ColorNode(Color.FromArgb(255, 235, 200, 110));
    [Menu("Current-step color", "Colour of the active step so it stands out")]
    public ColorNode CurrentColor { get; set; } = new ColorNode(Color.FromArgb(255, 120, 220, 255));
    [Menu("Optional color")] public ColorNode OptionalColor { get; set; } = new ColorNode(Color.FromArgb(255, 150, 150, 150));
    [Menu("Background color", "Alpha controls opacity; alpha 0 = no panel background")]
    public ColorNode BackgroundColor { get; set; } = new ColorNode(Color.FromArgb(140, 8, 8, 12));
    [Menu("Border color")] public ColorNode BorderColor { get; set; } = new ColorNode(Color.FromArgb(180, 90, 90, 110));
...
public class BannerStyle
{
    ...
    public ColorNode BackgroundColor { get; set; } = new ColorNode(Color.FromArgb(170, 8, 8, 12));
    [Menu("Border color")] public ColorNode BorderColor { get; set; } = new ColorNode(Color.FromArgb(200, 120, 220, 255));
...
public class ToastSettings
{
    ...
    [Menu("Text color")] public ColorNode TextColor { get; set; } = new ColorNode(Color.White);
    [Menu("Background color", "Alpha controls opacity")]
    public ColorNode BackgroundColor { get; set; } = new ColorNode(Color.FromArgb(215, 8, 8, 12));
    [Menu("Border color")] public ColorNode BorderColor { get; set; } = new ColorNode(Color.FromArgb(160, 120, 120, 160));
...
// corner markers drawn on inventory items that belong to the active build: colour by how close you are to
// the planned equip level, grey once the item has been equipped (detected from worn gear).
[Submenu]
public class GearIndicatorSettings
{
    [Menu("Enabled", "Draw a corner marker on build items in your inventory")]
    public ToggleNode Enable { get; set; } = new ToggleNode(true);

    [Menu("Marker size", "Corner triangle size in pixels")]
    public RangeNode<float> Size { get; set; } = new RangeNode<float>(14f, 6f, 40f);

    [Menu("Soon window (levels)", "Yellow when the planned level is within this many levels; red beyond it")]
    public RangeNode<int> SoonWindow { get; set; } = new RangeNode<int>(3, 1, 20);

    [Menu("Equippable now")] public ColorNode EquippableColor { get; set; } = new ColorNode(Color.FromArgb(255, 90, 200, 90));
    [Menu("Soon")] public ColorNode SoonColor { get; set; } = new ColorNode(Color.FromArgb(255, 230, 200, 70));
    [Menu("Later")] public ColorNode LaterColor { get; set; } = new ColorNode(Color.FromArgb(255, 220, 80, 70));
    [Menu("Used (equipped)")] public ColorNode UsedColor { get; set; } = new ColorNode(Color.FromArgb(255, 130, 130, 130));
}
...
public class ExileCampaignsSettings : ISettings
{
    ...
    [Menu("Build helper", "Build items/gems that are equippable now or become available next level")]
    public OverlayStyle BuildHelper { get; set; } = new OverlayStyle { PosX = new RangeNode<int>(320, 0, 4000), PosY = new RangeNode<int>(300, 0, 2160) };
    ...
    [Menu("Build item indicators", "Corner markers on inventory items that are part of your build")]
    public GearIndicatorSettings GearIndicators { get; set; } = new GearIndicatorSettings();
    ...
    [Menu("Add hovered item to build", "While hovering an item (gear or gem) in-game, press to add it to " +
        "this character's build at a target level. Level prefills from the item's requirement.")]
    public HotkeyNodeV2 AddGearKey { get; set; } = new HotkeyNodeV2(Keys.None);
