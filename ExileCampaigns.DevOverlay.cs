...
// All guarded on Settings.Dev.ShowDevOverlay (off by default).
//
// Layers (each toggled independently under Dev settings):
//   Room names    AreaGraph room boxes + name labels (the tile patterns Radar matches against).
//   Entity labels AreaTransition / Waypoint / boss entity paths at their grid positions.
...
public partial class ExileCampaigns
{
    ...
    {
        // editor's "Preview target" marker reuses this projection, so let it through even when the dev
        // overlay proper is off (room/entity layers stay gated on ShowDevOverlay).
        ...
        // target marker is editor-preview only
    ...
    private void DrawDevTargetMarker(Vector2 target, Vector2 playerGrid, float playerHeight, Vector2 mapCenter, float mapScale)
    {
        ...
        Graphics.DrawBox(screen - new Vector2(r, r), screen + new Vector2(r, r), Color.FromArgb(50, 0, 255, 0));
