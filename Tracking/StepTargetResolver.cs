...
public sealed class StepTargetResolver
{
    ...
    {
        // an explicit per-step pathTarget is always honored -- deliberate authoring, including for OPTIONAL far
        // bosses whose arena tile we want a path to (e.g. Balbala behind the seal door). without one, an
        // optional step never borrows the area's boss tile.
    ...
    // child via the PathTilePatterns -> ResolveTiles fallback in Pathing.
        ...
        Func<string, int, Vector2[]>? clusterTarget, string? effectiveAreaId)
    {
        ...
        // not loaded yet: the from-entry tile path comes from the PathTilePatterns -> ResolveTiles fallback in Pathing.
        if (clusterTarget == null) return null;

        // fallback: area-keyed JSON map.
    ...
    {
        // meta-first: step carries an embedded objective.
    ...
    public IReadOnlyList<Vector2> ResolveObjectiveTargets(GameController gc, ParsedStep step, string? effectiveAreaId)
    {
        ...
        // meta-first: step carries rooms directly.
        if (step.Meta?.Objective != null)
        {
            var mo = step.Meta.Objective;
    ...
    // ALL AreaGraph room centers whose name contains ANY of the given Room Path-child patterns. reads the
    // model's Room children directly, so rooms route for every objective type and across objectives.
    ...
    public IReadOnlyList<string> ResolveObjectiveProgressFlags(ParsedStep step, string? effectiveAreaId)
    {
        ...
        // meta-first: progress flags come directly off the step.
        if (step.Meta?.Objective != null)
        {
    ...
    public string? ObjectiveEntityPath(ParsedStep step, string? effectiveAreaId)
    {
        ...
        // meta-first: entity path comes directly off the step.
    ...
    public IReadOnlyList<Vector2> ObjectiveEntityPositions(GameController gc, IReadOnlyList<string> entityPaths, bool liveOnly)
    {
        ...
        foreach (var e in entities)
        {
            ...
            // WorldItems vanish when looted; isTargetable doesn't change. skip the interactable check for them.
    ...
    public IReadOnlyList<InteractTarget> ResolveIndicatorEntities(GameController gc, IEnumerable<Target> targets, float maxDistance)
    {
        ...
        foreach (var e in entities
            .Where(e => e != null && e.IsValid && HasGrid(e))
            .Where(e => e.DistancePlayer <= maxDistance)
            // WorldItems (ground drops) bypass the interactable check -- they aren't targetable in the
            // component sense but are valid arrow targets. IsInteractable exists to drop inactive NPC copies.
            .Where(e => e.Type == EntityType.WorldItem || IsInteractable(e))
        ...
        string? effectiveAreaId)
    {
        ...
        // meta-first: step carries the tile pattern directly.
    ...
    private static Vector2? NearestByPath(IEnumerable<ExileCore2.PoEMemory.MemoryObjects.Entity> entities, string pathFragment, Vector2? playerGrid)
    {
        var match = entities
    ...
    private static bool NameMatches(ExileCore2.PoEMemory.MemoryObjects.Entity e, string name)
    {
        var n = name.Trim();
    ...
    private static bool IsKillableCreature(ExileCore2.PoEMemory.MemoryObjects.Entity e)
    ...
    private static bool IsLiving(ExileCore2.PoEMemory.MemoryObjects.Entity e)
        => e.IsAlive;

    private static bool HasGrid(ExileCore2.PoEMemory.MemoryObjects.Entity e)
    ...
    private static string? MatchPath(ExileCore2.PoEMemory.MemoryObjects.Entity e)
        => e.Type == EntityType.WorldItem
            ? (e.GetComponent<WorldItem>()?.ItemEntity?.Path ?? e.Path)
            : e.Path;

    private static Vector2 GridOf(ExileCore2.PoEMemory.MemoryObjects.Entity e)
    {
