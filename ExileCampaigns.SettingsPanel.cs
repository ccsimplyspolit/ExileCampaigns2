...
public partial class ExileCampaigns
{
    private void DrawSettingsTabs()
    {
        ...
        if (ImGui.BeginTabItem("Overlays"))
        {
            ...
            ImGui.SeparatorText("Build helper");
            DrawOverlayStyle(Settings.BuildHelper, "buildhelper");
            ImGui.EndTabItem();
        }
        ...
        if (ImGui.BeginTabItem("Path & Indicator"))
        {
            ...
            Toggle("Highlight shortest path", p.HighlightShortest, "When a step draws several paths, tint the shortest a different color");
            if (p.HighlightShortest.Value)
                ColorEdit("Shortest path color", p.ShortestPathColor);
            ...
            SliderInt("Lookahead steps##mmi", mi.Lookahead, "Draw icons only for the current step and the next N steps in this area (0 = current only)");
            ImGui.EndTabItem();
        }
        ...
        if (ImGui.BeginTabItem("Alerts"))
        {
            ...
            ImGui.SeparatorText("Build item indicators");
            var g = Settings.GearIndicators;
            Toggle("Enabled##gear", g.Enable, "Draw a corner marker on build items in your inventory");
            SliderFloat("Marker size##gear", g.Size, "Corner triangle size in pixels");
            SliderInt("Soon window (levels)##gear", g.SoonWindow, "Yellow when planned level is within this many; red beyond");
            ColorEdit("Equippable now##gear", g.EquippableColor);
            ColorEdit("Soon##gear", g.SoonColor);
            ColorEdit("Later##gear", g.LaterColor);
            ColorEdit("Used (equipped)##gear", g.UsedColor);
            ImGui.EndTabItem();
        }
        ...
        if (ImGui.BeginTabItem("Hotkeys"))
        {
            ...
            if (ImGui.BeginTable("##hk_table", 2, ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.PadOuterX))
            {
                ...
                HotkeyRow("Add hovered item to build", Settings.AddGearKey,
                    "While hovering an item in-game, press to add it to this character's build at a target level");
                ImGui.EndTable();
            }
            ImGui.EndTabItem();
        }
    ...
    private static void ColorEdit(string label, ColorNode n, string? tip = null)
    {
        ...
        if (ImGui.ColorEdit4(label, ref v, ImGuiColorEditFlags.AlphaBar | ImGuiColorEditFlags.AlphaPreviewHalf))
            n.Value = Color.FromArgb(Byte(v.W), Byte(v.X), Byte(v.Y), Byte(v.Z));
        Tip(tip);
    }
