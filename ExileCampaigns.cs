...
// column stays put as step numbers grow. IsHeader lines (act/stage labels) skip the number column.
internal readonly struct PanelLine
{
    ...
    public PanelLine(string text, System.Drawing.Color color, bool isHeader = false, string? num = null,
...
public partial class ExileCampaigns : BaseSettingsPlugin<ExileCampaignsSettings>
{
    ...
    // the editor's selected step (RouteStep.Id), independent of the tracker's Current. survives a store reload.
    ...
    {
        // globalRules parsed from overrides.json (bundled first, user appends), used by waypoint-pulse
        // detection. they're global flags not attached to any step.
        _globalRules.Clear();
    ...
    // rebuild the read-side route from the edit store after an in-game edit. keeps the tracker on its current
    // step (clamped) and points the editor selection at selectId if it still exists. no disk re-read.
    ...
    public override void AreaChange(AreaInstance area)
    {
        try
        {
            ...
            {
                _route.OnAreaChanged(_areaId);   // zone forward-snap
            }
    ...
    // single advance gate: the current step's objectives, evaluated against live state.
    ...
    public override void Tick()
    {
        ...
        if (Settings.AddGearKey.PressedOnce()) OnAddGearPressed();
    ...
    public override void Render()
    {
        ...
        DrawGearDialog();   // modal popup, independent of overlay visibility
        ...
        float screenCenterX = GameController.Window.GetWindowRectangleTimeCache.Size.X / 2f;
        ...
        if (Settings.BuildHelper.Enable)
            DrawOverlay("buildhelper", BuildHelperLines(Settings.BuildHelper), Settings.BuildHelper, leftOpen, rightOpen, screenCenterX);
    ...
    private List<PanelLine> BuildStepsLines()
    {
        ...
        var dim = Color.FromArgb(170, s.TextColor.Value);
        var sepColor = Color.FromArgb(190, s.HeaderColor.Value);
        int lastAct = -1;
        ...
        bool leftOpen, bool rightOpen, float screenCenterX)
    {
        ...
        {
            var hint = (active || hovered) ? Color.FromArgb(220, 120, 220, 255) : Color.FromArgb(120, 120, 220, 255);
            ...
            if (onEdge || _resizeId == id)
                dl.AddLine(new Vector2(max.X, min.Y), new Vector2(max.X, max.Y), U32(Color.FromArgb(255, 120, 220, 255)), 3f);
        }
    ...
    // (which ExileCore2 honours), swallowing drag clicks before the game sees them.
