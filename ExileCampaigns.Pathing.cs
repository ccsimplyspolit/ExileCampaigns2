...
public partial class ExileCampaigns
{
    ...
    // the area's lifetime, so re-asking a known target yields nothing. completing a sub-objective just hides
    // its line (the bool[] below), it does not cancel/re-add anything.
    ...
    // drops the room filter, so "*" matches the whole map).
    ...
    private bool UpdateObjectivePaths(ParsedStep step, string? areaId)
    {
        ...
        // entity-only path children route to matching entities regardless of All/Nearest mode. they don't
        // use Radar tiles, so the mode distinction doesn't apply. WorldItems (ground drops) vanish when
        // looted, so MarkObjectiveDoneNotIn clears the path naturally without needing isTargetable.
    ...
    private void DrawStepPath()
    {
        ...
        // gather every path we'll draw this frame so we can flag the shortest. with only the single _currentPath
        // there's nothing to compare, so the highlight is a no-op (ShortestIndex stays on the lone line, same color
        // visually since there's no other to contrast -- but we only recolor when count > 1).
    ...
    private void DrawPathGround(List<Vector2i> path, float[][] hd, System.Drawing.Color color)
    {
    ...
    private void DrawPathMinimap(List<Vector2i> path, float[][] hd, SubMap largeMap, System.Drawing.Color color)
    {
        var pos = GameController.Player?.GetComponent<Positioned>();
    ...
    private void DrawOneComet(Vector2[] pts, float[] cum, float d, float size,
        float[][] hd, Camera cam, nint texId, System.Drawing.Color color, float maxEdge)
    {
        int seg = 1;
        ...
        // head sits at pos; quad runs 0.7 back (tail) + 0.3 forward, half-width each side. matches the sprite
        // layout (head ~70% to the right). corner order a,b,c,d = uv (0,0)(1,0)(1,1)(0,1)
        Vector2 baseB = pos - 0.7f * size * fwd;
