...
public partial class ExileCampaigns
{
    ...
    // once per tick before advance eval. detects newly-dead/non-interactable matched entities + open
    // dialogs for the step's matchers; resets on step change.
    ...
    private static bool MatchesAnyEntity(ExileCore2.PoEMemory.MemoryObjects.Entity e, IReadOnlyList<EntityMatcher>? ms)
    {
        ...
        try
        {
            ...
            foreach (var holder in invs)
            {
                ...
                foreach (var e in items)
                {
                    ...
                    if (snap.Valid && !string.IsNullOrEmpty(snap.Name)) result.Add(snap.Name);
                }
            }
        }
